import logging
import requests
import os  # <--- Added this to read settings from Render
...
app = FastAPI(title="Toxicity Guardian API", version="6.0.0")
...
# 3. HUGGING FACE API CONFIG (Remote Brain)
API_URL = "https://api-inference.huggingface.co/models/unitary/multilingual-toxic-xlm-roberta"

# SECURE TOKEN HANDLING:
# We get the token from the Server Environment, not this file.
...
@app.get("/health")
def health_check():
    return {"status": "active", "mode": "cloud_api"}

@app.post("/analyze")
async def analyze_text(request: AnalysisRequest):
    # --- CHECK 1: Manual Blacklists (Fastest) ---
    text_lower = request.text.lower()
    
    for bad_word in HINGLISH_BLACKLIST:
        if bad_word in text_lower:
            print(f"🚫 BLOCKED by Hinglish List: Found '{bad_word}'")
            return {"is_toxic": True, "categories": ["insult (manual)"]}

    for bad_word in HINDI_BLACKLIST:
        if bad_word in request.text: 
            print(f"🚫 BLOCKED by Hindi List: Found '{bad_word}'")
            return {"is_toxic": True, "categories": ["insult (hindi manual)"]}

    # --- CHECK 2: Hugging Face Cloud API ---
    try:
        if not HF_TOKEN:
            print("⚠️ WARNING: HF_TOKEN is missing! Check Render Environment Variables.")
            return {"is_toxic": False, "categories": ["config_error"]}

        print(f"☁️ Sending to AI Cloud: '{request.text[:30]}...'")
        response = requests.post(API_URL, headers=headers, json={"inputs": request.text})
        
        # If model is loading (503), assume safe temporarily
        if response.status_code == 503:
             print("⏳ Model is loading on cloud... assuming safe for now.")
             return {"is_toxic": False, "categories": []}
        ...
        if isinstance(results, list) and len(results) > 0:
            ...
            for res in scores:
                ...
                if label != 'neutral' and score >= request.threshold:
                    toxic_categories.append(label)
                    is_toxic = True
            
            print(f"   => AI VERDICT: {'TOXIC 🔴' if is_toxic else 'SAFE 🟢'}")
            return {"is_toxic": is_toxic, "categories": toxic_categories}
            
        return {"is_toxic": False, "categories": []}

    except Exception as e:
        logger.error(f"Cloud API Error: {e}")
        return {"is_toxic": False, "categories": ["error_bypass"]}